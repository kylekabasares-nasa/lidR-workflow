#!/usr/bin/env Rscript
# LiDAR processing pipeline for HPC (RStudio)
# STRICT ADHERENCE to lidR for DTM/DSM and terra for Slope/Aspect

# ============================================================================
# — PRELOAD udunits2 & REBUILD .libPaths() —
# ensure R can find libudunits2 before any package tries to load it
Sys.setenv(
  LD_LIBRARY_PATH = paste(
    Sys.getenv("LD_LIBRARY_PATH"),
    "/home5/kkabasar/.conda/pkgs/libudunits2-2.2.28-h40f5838_3/lib",
    sep = ":"
  )
)
# force the dynamic loader to pull in the .so now
dyn.load("/home5/kkabasar/.conda/pkgs/libudunits2-2.2.28-h40f5838_3/lib/libudunits2.so.0")

# rebuild .libPaths() so we keep your custom folder and the system defaults
custom_libs  <- c(
  "/home5/kkabasar/R-packages",
  "/vast_swbuild/swbuild3/kkabasar/pdal_lidR/lib/R/library"
)
default_libs <- .libPaths()
.libPaths(unique(c(custom_libs, default_libs)))
...
# Silence lasR header info
options(lasR.print.header = FALSE)
# ============================================================================
...
input_dir <- "/nobackupp27/nex/datapool/usgs-point-cloud-CA/CA_SanFrancisco_1_B23"  # Directory containing .laz files
output_dir <- "/nobackupp27/kkabasar/public/3DEP_v2/CA_SanFrancisco_1_B23"  # Where to save outputs

# Processing settings
num_cores <- 5  # Adjust based on your local machine (use detectCores() - 1 for auto)
# ============================================================================
